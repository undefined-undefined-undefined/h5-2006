...
$('.main-left button').on('click',async() => {
    ...
    const { code, nickname } = await $.post('../servers/login.php', { username, password }, null, 'json')
...
$('#textname').on('blur',async()=>{
    ...
    const {code} = await $.post('../servers/zcyz.php', { textname }, null,'json')
...
$('.main-zc .btn').on('click',async ()=>{
    ...
    const { code } = await $.post('../servers/zc.php',{ textname,textpsw,textnick },null,'json') 
    if(code) {
        alert('注册成功')
    }
  })
